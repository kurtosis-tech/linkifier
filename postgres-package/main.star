...
def run(
    ...
    # pga hba config
    pga_hba_str = read_file(src="pg_hba.conf.tmpl")
    postgresql_conf_str = read_file(src="postgresql.conf.tmpl")
    pga_hba_artifact = plan.render_templates(
        config={
            "pg_hba.conf": struct(
                template=pga_hba_str,
                data={},
            ),
            "postgresql.conf": struct(
                template=postgresql_conf_str,
                data={},
            )
        }
    )
    files["/data/hba_config"] = pga_hba_artifact
    ...
    plan.print("postgres command {0}".format(cmd))
